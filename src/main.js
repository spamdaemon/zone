...
(function() {
  ...
  var ensureMinArgs = function(args, min) {
    if (!(args.length >= min)) {
      throw new Error('Expected at least ' + min + ' arguments, but got ' +
          args.length);
    }
  };
   ...
   *
   * @param {!Array|!Arguments}  args the arguments
   * @param {!number}           min the minimum number of arguments
  ...
  var checkArguments = function(args, min, max) {
    ...
    if (!(args.length <= max)) {
      throw new Error('Expected at most ' + max + ' arguments, but got ' +
          args.length);
    }
  };
   ...
   * @throws {Error} if the name is already bound
   */
   ...
   *
   * @param {function(...[*])} f a function taking an variable number
   * of parameters
  ...
  var parseFormalParameters = function(f) {
    ...
    if (args === null) {
      console.log('Failed to parse the function (perhaps it is too long?) : ' +
          fnString);
    }
   ...
   * @constructor
   * @final
   * @param {!string} name the name of the module within the parent
   * @param {?Module} parent module
   * @param {Array=} opt_imports the imported modules
   */
  var Module = function(name, parent, opt_imports) {
    ...
    if (parent) {
      if (parent.__children[name]) {
        throw new Error('Module ' + this.__fullName +
            ' already contains a module ' + name);
      }
   ...
   *
   * @param {Module} source a module that needs access to the target module
   * @param {Module} target a module
   ...
   *
   * @param {!Module} m a module
   * @throws {Error}  if the module is already sealed
   */
  ...
  /**
   * Find a module by searching either from the start or the root. A search is
   * performed from the root if the name is an absolute name, i.e. contains .
   *
   ...
   *
   * @param {!Array} names the function names
   ...
   * @constructor
   * @final
   * @param {Array|Arguments} args an array of arguments
   */
   ...
   * @constructor
   * @final
   * @param {!Module} module the module in which the interceptor will resolve
   * @param {!FunctionDescriptor} descriptor the function used to create the
   * interceptor
   */
   ...
   *
   * @param {*|ValueDescriptor}  value a value
   *
   ...
   * Create the function description.
   *
   * @param {!Array|!Arguments|!FunctionDescriptor} args an array of arguments
   ...
   *
   * @param {!Array|!Arguments|!FunctionDescriptor} args a function or a
   * descriptor
   * @return {FunctionDescriptor} a function descriptor
   */
   ...
   * Create a descriptor.
   *
   * @param {Array|ValueDescriptor|FunctionDescriptor} args an argument array
  ...
  var guessDescriptor = function(args) {
    ...
    if ((args.length === 1) && (typeof args[0] !== 'function') &&
        !(args[0] instanceof FunctionDescriptor)) {
   ...
   * @constructor
   * @final
   * @param {!Module} module the module where this resolvable will live
   * @param {!string} name the name of the resolvable
   * @param {number} access the access for this resolvable
   * @param {!FunctionDescriptor|!ValueDescriptor} descriptor a function or
   * value descriptor
   */
   ...
   *
   * @param {!number}  access a access level
   * @return {boolean} true if this resolvable has at an access level of
   * 'access'
   */
  ...
  /**
   * Define a private object which is only accessible to functions defined in
   * the provided module.
   *
   * @param {!Module} module the module in which to bind a new object
   * @param {!string} name the name of the object to be bound
   * @param {!number} access the access level
   * @param {!Array} args objects to bind or a function descriptor
   ...
   * @constructor
   * @final
   * @param {!string} path a path name to parse
   * @param {!Module} module the default module
   */
   ...
   *
   * @param {!string} name the simple or absolute name of the resolvable
   * @param {!Module} start the module in which to start searching
   * @param {number} access the type access granted to the module's resolvables
   * @param {!Object} recursionGuard the recursion guard is necessary to detect
   * cyclic dependencies
      ...
      if (!resolvable) {
        ...
        try {
          ...
          for (i = 0, n = imports.length; i < n && !resolvable; ++i) {
            ...
            resolvable =
                findResolvable(local, depends, PUBLIC_ACCESS, recursionGuard);
          }
        } finally {
  ...
  /**
   * Inject a function with objects from the specified module. The this pointer
   * will be mapped to the module.
   *
   * @param {!Module}  module the module
   * @param {number} access the access
   * @param {FunctionDescriptor} descriptor the descriptor for the function
   * @param {boolean} allowFreeArguments true to allow free arguments
   * @return {function()|null} a function that calls the specified function
   * with the appropriately injected values or
   *         null if the injection failed
   * @throws Error if a cyclic dependency was detected
   */
  var injectFunction = function(module, access, descriptor,
                                allowFreeArguments) {
   ...
   *
   * @param {!Resolvable} R the resolve to be instantiated
  ...
  var resolveValue = function(R) {
    ...
    for (i = 0, n = interceptors.length; i < n; ++i) {
      ...
      try {
        interceptFN = injectFunction(interceptor.module, PRIVATE_ACCESS,
            interceptor.descriptor, false);
      } catch (error) {
  ...
  /**
   * Inject a function with values from this module. The this pointer will be
   * bound to this module.
   *
   * @expose
   * @param {...} var_args a function descriptor
  ...
  /**
   * Create a new module that is nested within the current module. If the
   * modules has already been created with
   * imports, then this method throws an exception.
   *
   * @expose
   * @param {!string} name the name of this module within the parent module
   */
   ...
   *
   * @expose
   * @param {Array=} opt_imports direct imports of this module
   */
  ...
  /**
   * Get the value of a property by its local or absolute path. Only public
   * values can be retrieved.
   *
   * @expose
   * @param {!string} name the name of the object to be resolved.
   * @return {*} a value
   * @throws Exception
  ...
  Module.prototype.definePrivate = function(name, var_args) {
    ...
    return define(this, name, PRIVATE_ACCESS,
        Array.prototype.slice.call(arguments, 1));
  };

  /**
   * Define a private object which is only accessible to functions defined in
   * this module or nested modules.
   *
   * @expose
   * @param {!string} name the name of the object to be bound
   * @param {...*} var_args
  ...
  Module.prototype.defineProtected = function(name, var_args) {
    ensureMinArgs(arguments, 2);
    return define(this, name, PROTECTED_ACCESS,
        Array.prototype.slice.call(arguments, 1));
  };

  /**
   * Associate a value with a public name. If the arguments can be interpreted
   * as a factory function, then the
  ...
  Module.prototype['export'] = function(name, var_args) {
    ensureMinArgs(arguments, 2);
    return define(this, name, PUBLIC_ACCESS,
        Array.prototype.slice.call(arguments, 1));
  };
  ...
  Module.prototype.factory = function(name, var_args) {
    ensureMinArgs(arguments, 2);
    var desc = createFunctionDescriptor(Array.prototype.slice.call(arguments,
        1));
  ...
  Module.prototype.service = function(name, var_args) {
    ensureMinArgs(arguments, 2);
    var desc = createConstructorDescriptor(Array.prototype.slice.call(arguments,
        1));
    return define(this, name, PRIVATE_ACCESS, [desc]);
  };
  ...
  /**
   * Define an interceptor for values, factories, and services. The interceptor
   * is invoked when the named object in this module is resolved for the first
   * time. The interception function can be injected and must return a function
   * that can be used to inject.
   *
   * @expose
   * @param {!string} name the name of the object to be intercepted
   * @param {...} var_args an injectable function that produce a function that
   ...
   *
   * @expose
   * @param {!string=} opt_path the optional path
   * @param {boolean=} opt_preventImplicitModule true to prevent the module from
  ...
  /**
   * Create a function descriptor. This value may be passed to any of the
   * define functions or the inject function.
   *
   * @expose
   * @param {...*} var_args
   * @return {FunctionDescriptor} a function descriptor
  ...
  /**
   * Create a descriptor for a constructor function. Functions defined as
   * constructors will be instantiated using the
  ...
  /**
   * Create a descriptor for a value. This function can be used in some
   * circumstances to disambiguate different
  ...
  /**
   * An injection function. This works much like zone(name).inject(...). It's
   * very useful to this use this during
   * testing.
   *
   * @expose
   * @param {!string=} opt_name the name of optional module which to use for
   * injection
